(function () {
    var options = {
        "id": 12097,
        "hash": "e9670015be3a36a92c0c91387c348061b4e902a17ce5fb7f662bd602add44bc9",
        "siteUrl": "https:\/\/victoria.libanswers.com",
        "iid": 2208,
        "onlinerules": [{
            "u": 0,
            "d": [3860],
        ...
        "slidebutton_bcolor": "#3E9C4F",
        "slidebutton_color": "#FFFFFF",
        "slidebutton_bcolor_off": "#3E9C4F",
    ...
    var removeCoverageFromRule = function removeCoverageFromRule(rule) {
        ...
        adjustStatusRequestForCoverage = function adjustStatusRequestForCoverage(onlineRules) {
            var finalRules = [],
                now = new Date(),
                day = now.getUTCDay(),
        ...
        isItDuringBotCustomHours = function isItDuringBotCustomHours(customHours) {
            var now = new Date(),
                day = now.getUTCDay(),
                hour = now.getUTCHours(),
                minute = now.getUTCMinutes(),
                times = customHours;
            if (!times || !times[day] || 0 === times[day].length) return !1;
            var ranges = times[day];
            return isCoverageInRange(hour, minute, ranges);
        },
        isBotActiveWithoutStatusCheck = function isBotActiveWithoutStatusCheck(botAvailability, customHours) {
            return 1 === botAvailability || 4 === botAvailability && isItDuringBotCustomHours(customHours);
        };
    ; var floatWidget = {
        config: {},
        online: !1,
        ...
        referer: "",
        refererTitle: "",
        boundKeyboardHandler: null,
        boundOverlayHandler: null,
        narrowScreen: null,
        ...
        hasLocationChanged: function hasLocationChanged() {
            if (this.referer != window.location.href) {
                return true;
            }
            return false;
        },
        getWidgetUrl: function getWidgetUrl() {
            ...
            var authId = 0;
            window.springSpace && window.springSpace.la && window.springSpace.la.Page && window.springSpace.la.Page.auth_id && (authId = window.springSpace.la.Page.auth_id);
            var qs = this.config.siteUrl + "/chat/widget/" + this.config.hash + "?referer=" + encodeURIComponent(this.referer) + "&referer_title=" + encodeURIComponent(this.refererTitle) + "&auth_id=" + authId + "&chat_online=" + this.online;
            return this.autoload && (qs += "&auto=true", this.autoload = !1), qs;
        },
        statusSuccess: function statusSuccess(data) {
            var online = !1;
            (data.u || data.d || data.c) && (online = !0), online || 2 !== this.config.botAvailability || (online = !0), this.changeButtonStatus(online);
            var isCoopOnline = !1;
            data.c && data.c.length > 0 && (isCoopOnline = !0), this.setTimer(isCoopOnline);
        },
        checkStatusCtx: function checkStatusCtx(context) {
            context.checkStatus();
        },
        checkLocation: function checkLocation(context) {
            if (context.hasLocationChanged()) {
                // Reset the referer and referer title
                context.referer = "", context.refererTitle = "";
                // rebuild the chat URL based on the parent window
                var chatUrl = context.getWidgetUrl();
                // close the chat widget
                context.closeWidget();
                // reload the chat widget
                context.chat_load.setAttribute("src", chatUrl);

                // Future developement trigger event on the widget to update the referer
                //this.chat_load.contentWindow.postMessage({call:'referer', value: this.referer},'*');
                //this.chat_load.contentWindow.postMessage({call:'refererTitle', value: this.refererTitle},'*');
            }
        },
        checkStatus: function checkStatus() {
            var _this = this;

            //  ------ COMMENTED OUT BY BF DUE TO NOT CHECKING THE CHAT AVAILABILITY
            //if (isBotActiveWithoutStatusCheck(this.config.botAvailability, this.config.botCustomHours))
            //    return this.changeButtonStatus(!0),
            //    void (this.config.botUseAutoload && this.setTimer(!1));
            ...
            xhr.onload = function () {
                ...
                if (status >= 200 && status < 300) try {
                    _this.statusSuccess(JSON.parse(xhr.responseText));
                } catch (e) {
                    _this.changeButtonStatus(!1);
                } else {
                    _this.changeButtonStatus(!1);
                }
                if (_this.chat_load) {
                    // Raise an event on the chat, -- See the look & feel for the javascript that handles it
                    _this.chat_load.contentWindow.postMessage({ call: 'chatStatus', value: _this.online }, '*');
                }
            }, xhr.onerror = function () {
                _this.changeButtonStatus(!1);
            }, xhr.open("GET", this.cascadeServer + "/widget_status?sessionTime=" + sessionTime + "&iid=" + this.config.iid + "&rules=" + encodeURIComponent(JSON.stringify(adjustedRules))), xhr.send();
        },
        ...
        handleMessages: function handleMessages(e) {
            var data = e.data;
            if ("object" == (typeof data === "undefined" ? "undefined" : _typeof(data)) && data.action) switch (data.action) {
                case "peelout":
                case "closeWidget":
                    this.chat_button.focus(), this.closeWidget();
                    break;
                case "chatStarted":
                    this.checkStatus();
                case "expandWidget":
                    this.boundOverlayHandler && this.overlay.removeEventListener("click", this.boundOverlayHandler), this.overlay.style.display = "none", this.chat_load.classList.contains("expanded") || (this.chat_load.classList.add("expanded"), this.chat_load.style.minHeight = 0);
                    break;
                case "autopop":
                    this.openWidget(), this.boundOverlayHandler = this.closeWidget.bind(this), this.overlay.addEventListener("click", this.boundOverlayHandler), this.overlay.style.display = "block";
                    break;
                case "height":
                    null !== this.chat_load && data.height && (this.chat_load.style.minHeight = data.height + "px");
                    break;
                case "newMessage":
                    "true" === this.chat_load.getAttribute("aria-hidden") && this.showMsgNotice();
                    break;
                case "changeStatus":
                    this.changeButtonStatus(data.online || !1);
                    break;
            }
        },
        closeWidget: function closeWidget() {
            this.chat_load.removeAttribute("aria-live"), this.chat_load.setAttribute("aria-hidden", "true"), this.chat_div.classList.remove("open"), this.chat_button.setAttribute("aria-label", this.config.translation.ch_float_show), this.chat_button.setAttribute("title", this.config.translation.ch_float_show), this.chat_button.setAttribute("aria-expanded", !1), this.chat_load.contentWindow.postMessage("closeChatWidget", this.config.siteUrl + "/"), document.body.removeEventListener("keydown", this.boundKeyboardHandler), this.boundOverlayHandler && this.overlay.removeEventListener("click", this.boundOverlayHandler), this.overlay.style.display = "none", this.checkStatus();
        },
        openWidget: function openWidget() {
            if (this.chat_div.classList.contains("open") || this.chat_div.classList.add("open"), this.chat_button.setAttribute("aria-label", this.config.translation.ch_float_hide), this.chat_button.setAttribute("title", this.config.translation.ch_float_hide), this.chat_button.setAttribute("aria-expanded", !0), this.hideMsgNotice(), this.boundKeyboardHandler = this.keyboardCloseWidget.bind(this), document.body.addEventListener("keydown", this.boundKeyboardHandler), null !== this.chat_load) {
                return this.chat_load.setAttribute("aria-hidden", "false"), void this.chat_load.focus();
                this.checkStatus();
            } else {
                this.loadIframe();
                window.setTimeout(this.checkStatusCtx, 500, this);
            }
        },
        keyboardCloseWidget: function keyboardCloseWidget(ev) {
            "Escape" === ev.key && this.closeWidget();
        },
        loadIframe: function loadIframe() {
            if (null !== this.chat_load) return;
            var chatUrl = this.getWidgetUrl();
            this.chat_load = document.createElement("iframe"), this.chat_load.setAttribute("id", "iframe_ " + this.config.hash), this.chat_load.setAttribute("name", "iframe_ " + this.config.hash), this.chat_load.setAttribute("src", chatUrl), this.chat_load.setAttribute("title", "Chat Widget"), this.chat_load.setAttribute("scrolling", "no"), this.chat_load.className = "s-lch-widget-float-load", this.chat_load.setAttribute("aria-live", "polite"), this.chat_load.setAttribute("aria-hidden", "false"), this.chat_load.innerHTML = "Content is loading...", this.chat_div.insertBefore(this.chat_load, this.chat_button), window.addEventListener("message", this.handleMessages.bind(this)), this.chat_load.contentWindow.postMessage({ call: 'chatStatus', value: this.online }, '*');
            this.checkStatus();
            window.setInterval(this.checkStatusCtx, 4000, this);
            window.setInterval(this.checkLocation, 500, this);
        },
        autoOpenWidget: function autoOpenWidget() {
            this.autoPopDenied() || (this.autoload = !0, this.loadIframe());
        },
        ...
        changeButtonStatus: function changeButtonStatus(online) {
            if (online !== this.online) {
                if (this.online = online, this.online) return this.chat_button.classList.add("online"), void (this.image ? (this.image.setAttribute("src", this.config.slidebutton_url), this.image.setAttribute("alt", this.config.slidebutton_text)) : this.svgTitle.innerHTML = this.config.slidebutton_text);
                this.chat_button.classList.remove("online"), this.image ? (this.image.setAttribute("src", this.config.slidebutton_url_off), this.image.setAttribute("alt", this.config.slidebutton_text_off)) : this.svgTitle.innerHTML = this.config.slidebutton_text_off;
            }
        },
        ...
        start: function start() {
            ...
            !0 !== this.loaded && (this.loaded = !0, this.insertWidgetCSS(), this.chat_div = document.createElement("div"), this.chat_div.id = "s-lch-widget-" + this.config.id, this.chat_div.setAttribute("role", "region"), this.chat_div.setAttribute("aria-label", "Chat Widget"), this.chat_div.className = "s-lch-widget-float", this.overlay = document.createElement("div"), this.overlay.classList.add("tab_overlay"), this.chat_div.appendChild(this.overlay), document.body.appendChild(this.chat_div), this.buildButton(), this.checkStatus(), document.addEventListener("visibilitychange", function () {
                "visible" !== document.visibilityState || _this2.chat_load || _this2.checkStatus();
            }));
        }
    };
